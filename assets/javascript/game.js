...
document.onkeyup = function(event) {
  ...
  // .. also executes the first time any key is pressed
   if (isPushed.indexOf(userGuess) === -1) {
        ...
        //The following block of statements will execute if more than one button has been pressed, 
        //AND the letter is in the letter array
        if ((totalClickCount > 1) && (letters.indexOf(userGuess.toUpperCase()) >= 0)) {
          ...
          title = '';             
                ...
                for (i=0; i<chosenMovie.length; i++) {
            	   //if the chosen letter is in the isPushed array, then concatentate the letter to 
                   //title, converts all to uppercase
        ...
        // The section below resets the play area and adds a win to the win counter if the 
        //player gets all the letters
